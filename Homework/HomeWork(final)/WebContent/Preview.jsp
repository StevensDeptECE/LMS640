          ...
          <div id = "Questions" class = "requirement">
             ...
             if(eobj.get("name").equals(Instructor)||eobj.get("name").equals(TA)){
            	%>
        	 	...
        	 	<hr />
            	<% 	
            	DBCollection thisassignment = dbC.getCollection(assignment);
             	DBCursor cursor = thisassignment.find();
				try{
					while(cursor.hasNext()){
						DBObject str = cursor.next();
						%>
		     			<h2><a href = "PreviewDetail.jsp?id=<%out.println(str.get("name"));%>"><%out.println(str.get("name"));%></a></h2>
		     	<%
		     		}
				}
				finally
