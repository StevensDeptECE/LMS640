...
settings.prototype.draw = function(s) {
	...
	q.draw(p);
    document.getElementById("submitQuiz-button").innerHTML = "Save";
    document.getElementsByClassName("quiz-nav-right")[0].style = "display:none";
    var qc = document.getElementsByClassName("qc");
    for(var i = 0; i < qc.length; i++){
        document.getElementsByClassName("qcPoints")[0].remove();
    }
}
    ...
    [
...

        {
            ...
            comp: [
                ...
                [ "MC",["Yes", "No"], "3"],
                ["Instr", "How many days between quiz open date and quiz due date?", "1"],
                [ "FillIn", "2"],
                ["Instr", "How many days between quiz due date and quiz close date?", "1"],
                [ "FillIn", "2"]
            ]
        }
    ]
